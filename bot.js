...
setInterval(() => {
  ...
  http.get(`http://beratkay.glitch.me`);
              ...
              if (reklam.some(word => msg.content.toLowerCase().includes(word))) {
                try {
                  if (!msg.member.hasPermission("MANAGE_WEBHOOKS")) {
                    ...
                    .setColor(0xffa300)
                    .setFooter('⅌ Alondor Reklam Sistemi', client.user.avatarURL)
                    .setAuthor(msg.guild.owner.user.username, msg.guild.owner.user.avatarURL)
                    .setDescription("⅌ Alondor , " + `***${msg.guild.name}***` + " adlı sunucunuzda reklam yakaladım.")
              ...
              if (küfür.some(word => msg.content.toLowerCase().includes(word))) {
                try {
                  if (!msg.member.hasPermission("MANAGE_WEBHOOKS")) {
                    ...
                    let embed = new Discord.RichEmbed()
                    .setColor(0xffa300)
                    .setFooter('⅌ AlondorSistemi', client.user.avatarURL)
                    .setAuthor(msg.guild.owner.user.username, msg.guild.owner.user.avatarURL)
                    .setDescription("⅌ Alondor , " + `***${msg.guild.name}***` + " Sunucunuzda Küfür Edildi..")
                    .addField('Küfür Eden Kişi', 'Kullanıcı: '+ msg.author.tag +'\nID: '+ msg.author.id, true)
                    .addField('Engellenen mesaj', msg.content, true)
                    .setTimestamp()                   
                    msg.guild.owner.user.send(embed)                       
                    return msg.channel.send(`${msg.author}, Küfür Etmek Yasak Kurucuya Özelden Yazdım.`).then(msg => msg.delete(25000));
                  }              
                } catch(err) {
                  console.log(err);
                }
...
client.on("channelDelete", async channel => {
  ...
  if(deleter.id == "749915024109207634") return; //bu satıra kendi id'nizi yazın sizin kanal silmenizi engellemeyecektir
